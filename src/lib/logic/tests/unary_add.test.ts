...
describe('valid doublon', () => {
    ...
    const testInputs = new Array(MAX)
        .fill(null)
        .map((_, index) => [...new Array((index + 1)).fill('1'), '+', ...new Array((index + 1)).fill('1'), '=']);

    for (const testInput of testInputs) {
        let sideLength = (testInput.length - 2) / 2;
        test(testInput.toString(), () => {
            const output = run(UnaryAddInstructionSet as InstructionSet, testInput, 1000);
            expect(output.tape.toArray()).toEqual([...new Array((sideLength * 2)).fill('1'), ...new Array((sideLength + 2)).fill('.')]);
        });
    }
});
...
describe('valid doublon plus one left', () => {
    ...
    const testInputs = new Array(MAX)
        .fill(null)
        .map((_, index) => [...new Array((index + 2)).fill('1'), '+', ...new Array((index + 1)).fill('1'), '=']);

    for (const testInput of testInputs) {
        let sideLength = Math.floor((testInput.length - 2) / 2);
        test(testInput.toString(), () => {
            const output = run(UnaryAddInstructionSet as InstructionSet, testInput, 3000);
            expect(output.tape.toArray()).toEqual([...new Array((sideLength * 2 + 1)).fill('1'), ...new Array((sideLength + 2)).fill('.')]);
        });
    }
});
...
describe('valid doublon plus one right', () => {
    ...
    const testInputs = new Array(MAX)
        .fill(null)
        .map((_, index) => [...new Array((index + 1)).fill('1'), '+', ...new Array((index + 2)).fill('1'), '=']);

    for (const testInput of testInputs) {
        let sideLength = Math.floor((testInput.length - 2) / 2);
        test(testInput.toString(), () => {
            const output = run(UnaryAddInstructionSet as InstructionSet, testInput, 3000);
            expect(output.tape.toArray()).toEqual([...new Array((sideLength * 2 + 1)).fill('1'), ...new Array((sideLength + 3)).fill('.')]);
        });
    }
});
...
describe('valid random', () => {
    ...
    for (const testInput of testInputs) {
        test(testInput.toString(), () => {
            ...
            expect(output.tape.toArray().slice(expected.length)).toEqual(new Array(output.tape.count() - expected.length).fill('.'));
        });
    }
});
