...
function visualize(
  ...
  const tapeShow =
    "[" +
    tape
      .map((symbol, index): string =>
        index === headPosition ? `<${symbol}>` : `${symbol}`,
      )
      .join("") +
    "]";
...
const step =
  ...
  (cycle: Cycle): Readonly<MachineOutput> => {
    ...
    const currentHeadPosition = moveHead(cycle.headPosition)(
      previousTransition.action,
    );
    const currentSymbol = tape.get(currentHeadPosition)!;

    if (
      finalReached(parameters.finals)(currentState) ||
      limitReached(cycle.limit)
    )
      return {
        tape,
    ...
    const nextTransition = getNextTransition(parameters.transitions)(cycle)(
      currentSymbol,
    );
    ...
    const nextTape = updateTape(tape)(currentHeadPosition)(
      nextTransition.write,
    );
