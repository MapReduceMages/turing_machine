...
<script lang="ts">
	...
	$: inputRespectAlphabet = $InstructionSetStore !== null && localInput.split('').every((char) =>  $InstructionSetStore.alphabet.includes(char));
</script>
...
<div class="mb-2 flex w-fit flex-col gap-box-sm md:w-full md:flex-row">
	<input type="text" spellcheck="false" placeholder="input tape" maxlength={Config.inputMaxLength} bind:value={localInput} disabled={$OutputStore !== null || $InstructionSetStore === null}/>
	<div class="flex w-full justify-center gap-box-sm md:justify-between">
		<button class="!w-fit pr-3" on:click={handleLoad} disabled={$OutputStore !== null || localInput.length === 0 || !inputRespectAlphabet}>
