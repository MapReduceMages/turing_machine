...
export const TransitionSchema = Joi.object({
  ...
  action: Joi.string()
    .valid(Config.action.leftLabel, Config.action.rightLabel)
    .required(),
});
...
export const InstructionSetSchema = Joi.object({
  ...
  alphabet: Joi.array()
    .items(Joi.string().min(1).max(Config.maxAlphabetLength))
    .required(),
  blank: Joi.string().min(1).max(Config.maxAlphabetLength).required(),
  states: Joi.array()
    .items(Joi.string().min(1).max(Config.maxStateLength))
    .min(1)
    .required(),
  ...
  transitions: Joi.object()
    .pattern(Joi.string().min(1), Joi.array().items(TransitionSchema))
    .required(),
});

export function checkInstructionSet(
  instructionSet: InstructionSet,
): Error | null {
  // Check if the alphabet contains the blank symbol
  if (!instructionSet.alphabet.includes(instructionSet.blank)) {
    return new Error(
      `The blank symbol ("${instructionSet.blank}") must be in the alphabet`,
    );
  }
  ...
  if (!instructionSet.states.includes(instructionSet.initial)) {
    return new Error(
      `The initial state ("${instructionSet.initial}") must be in the states`,
    );
  }
  ...
  for (const finalState of instructionSet.finals) {
    if (!instructionSet.states.includes(finalState)) {
      return new Error(
        `The final states ("${finalState}") must be in the states`,
      );
    }
  }
  ...
  for (const state in instructionSet.transitions) {
    for (const transition of instructionSet.transitions[state]) {
      ...
      if (!instructionSet.alphabet.includes(transition.read)) {
        return new Error(
          `The read symbol ("${transition.read}") must be in the alphabet`,
        );
      }
      ...
      if (!instructionSet.alphabet.includes(transition.write)) {
        return new Error(
          `The write symbol ("${transition.write}") must be in the alphabet`,
        );
      }
      ...
      if (!instructionSet.states.includes(transition.to_state)) {
        return new Error(
          `The to state ("${transition.to_state}") must be in the states`,
        );
      }
    }
  }
